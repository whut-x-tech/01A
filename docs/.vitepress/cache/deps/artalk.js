...
var markedLinkRenderer = (renderer, orgLinkRenderer) => (args) => {
...
var markedImageRenderer = (renderer, orgImageRenderer, { imgLazyLoad }) => (args) => {
  const html2 = orgImageRenderer.call(renderer, args);
  if (!imgLazyLoad) return html2;
...
function convertApiOptions(conf, ctx) {
  return {
    ...
    userInfo: (ctx == null ? void 0 : ctx.get("user").checkHasBasicUserInfo()) ? {
      name: ctx == null ? void 0 : ctx.get("user").getData().name,
...
var SubmitAddPreset = class {
  ...
  getSubmitAddParams() {
    return __async(this, null, function* () {
      const { name, email, link: link2 } = this.kit.useUser().getData();
  ...
  ctx.on("mounted", () => {
    timer = window.setInterval(() => {
      ...
      list2.$el.querySelectorAll("[data-atk-comment-date]").forEach((el) => {
        ...
        el.innerText = ((_b = (_a = ctx.getConf()).dateFormatter) == null ? void 0 : _b.call(_a, date)) || timeAgo(date, ctx.$t);
      });
    }, 30 * 1e3);
  });
  ctx.on("unmounted", () => {
...
var GlobalPlugins = /* @__PURE__ */ new Set([...DefaultPlugins]);
var PluginOptions = /* @__PURE__ */ new WeakMap();
function load(ctx) {
  return __async(this, null, function* () {
    var _a;
...
function onLoadErr(ctx, err) {
  ...
  showErrorDialog({
    ...
    errData: err.data,
    retryFn: () => load(ctx),
    onOpenSidebar: ctx.get("user").getData().is_admin ? () => ctx.showSidebar({
